...
def puzzle():
    return {"numbers": [2, 4, 6, 8, 10], "target": 8, "pick": 2, "solution": [0, 2]}
...
class TestGame(unittest.TestCase):
    ...
    def test_play_round_correct_hint_wrong_and_invalid(self):
        settings = base_settings()
        puzzles = [puzzle(), puzzle(), puzzle(), puzzle(), puzzle(), puzzle()]
        inputs = ["1 3", "hint", "1 3", "1", "1 2", "1 2", "1 2", "1 2", "1 2", "1 2", "1 2", "1 2"]
        with mock.patch.object(game.core, "make_puzzle", side_effect=puzzles), mock.patch("builtins.input", StackedInput(inputs)):
    ...
    def test_play_round_quits(self):
        settings = base_settings()
        with mock.patch.object(game.core, "make_puzzle", return_value=puzzle()), mock.patch("builtins.input", StackedInput(["q"])):
